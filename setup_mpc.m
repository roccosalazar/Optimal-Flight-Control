...
mpc_obj.PredictionHorizon = 10;  % Numero di passi di predizione
...
mpc_obj.Weights.ManipulatedVariablesRate = [0.01 0.01];  % Penalizzazione sulla variazione di u
mpc_obj.Weights.OutputVariables = [1 1 0 0 0];  % Penalizzazione sugli stati (solo x, y)

% Vincoli sugli input (forza massima applicabile)
mpc_obj.MV(1).Min = -500;  % Forza minima su x
mpc_obj.MV(1).Max = 500;   % Forza massima su x
mpc_obj.MV(2).Min = -500;  % Forza minima su y
mpc_obj.MV(2).Max = 500;   % Forza massima su y
